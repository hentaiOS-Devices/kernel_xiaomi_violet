...
static int update_tp_info(struct i2c_client *client)
{
	char tp_info_buf[64];
	u8 ver = 0;
...
void fts_fwupg_auto_upgrade(struct fts_ts_data *ts_data)
{
	...
	ts_data->touch_state = TOUCH_STATE_UPGRADING;
	...
	ts_data->touch_state = TOUCH_STATE_WORKING;
